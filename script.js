...
menuIcon.addEventListener('click', (event) => {
    ...
    // Change the icon to 'X' or back to the menu
    if (isMenuOpen) {
        menuIcon.innerHTML = '<i class="bx bx-x"></i>'; // Change to 'X' icon
        navLinks.style.display = 'block'; // Show the menu
    } else {
        menuIcon.innerHTML = '<i class="bx bx-menu"></i>'; // Change back to menu icon
...
window.addEventListener('resize', () => {
    if (window.innerWidth > 768) {
        ...
        menuIcon.innerHTML = '<i class="bx bx-menu"></i>'; // Reset icon to menu
    } else {
